...
public class AlloyFurnaceContainer extends AbstractMachineContainer
{
    public AlloyFurnaceContainer(final int windowId, final PlayerInventory playerInv, final AlloyFurnaceTileEntity te)
    {
        super(ContainerInit.ALLOY_FURNACE_CONTAINER.get(), windowId, te);
        ...
        this.addSlot(new Slot(te, 1, 55, 41)
        {
            ...
            public boolean isItemValid(ItemStack stack)
            {
                return Recipes.ALLOYS.keySet().stream().anyMatch(pair -> pair.getFirst().getItem() == stack.getItem());
            }
        });
        ...
        this.addSlot(new Slot(te, 2, 55, 21)
        {
            ...
            public boolean isItemValid(ItemStack stack)
            {
                return Recipes.ALLOYS.keySet().stream().anyMatch(pair -> pair.getSecond().getItem() == stack.getItem());
            }
        });
    ...
    private static AlloyFurnaceTileEntity getTileEntity(final PlayerInventory playerInv, final PacketBuffer data)
    {
        ...
        {
            return (AlloyFurnaceTileEntity) te;
        }
