...
public class SteamhammerContainer extends AbstractMachineContainer
{
    public SteamhammerContainer(final int windowId, final PlayerInventory playerInv, final SteamhammerTileEntity te)
    {
        super(ContainerInit.STEAMHAMMER_CONTAINER.get(), windowId, te);
        ...
        this.addSlot(new Slot(te, 1, 55, 41)
        {
            ...
            public boolean isItemValid(ItemStack stack)
            {
                return Tags.Items.INGOTS.getAllElements().contains(stack.getItem());
            }
        });
    ...
    public SteamhammerContainer(final int windowId, final PlayerInventory playerInv, final PacketBuffer data)
    {
        this(windowId, playerInv, getTileEntity(playerInv, data));
    }

    private static SteamhammerTileEntity getTileEntity(final PlayerInventory playerInv, final PacketBuffer data)
    {
        Objects.requireNonNull(playerInv, "Player inventory cannot be null");
        Objects.requireNonNull(data, "PacketBuffer cannot be null");
        final TileEntity te = playerInv.player.world.getTileEntity(data.readBlockPos());
        if (te instanceof SteamhammerTileEntity)
        {
            return (SteamhammerTileEntity) te;
        }
